...
【執筆のトーン＆マナー】
- 産婦人科専門医として、冷静かつ客観的・中立的な立場で、読者に正確な情報を提供するトーン。過度に感情的（「愛情を込めて」等）な表現は避ける。
...
4. 【中立性の徹底・誇大表現の完全禁止（薬機法・景表法・YMYL対応）】
   過度な期待を抱かせる煽り表現や断定表現は絶対に禁止。
   以下のワードはJP/EN記事ともに使用禁止:
   ...
   代替表現: 「〜の可能性が示唆されている」「〜に寄与する可能性がある」/ "may help", "research suggests", "has been associated with"
...
`;
    ...
    console.log('🤖 Firing Gemini API with retry for text generation...');
    ...
    console.error('❌ Auto Generator (TEXT) Error:', error);
    ...
    await sendSlackErrorAlert(slackToken, slackChannel, 'auto-generator-text', error.message || String(error));
