...
1. "jpBlog": 完全な日本語版のMDXブログ記事（2000文字程度）。
   ...
   - 【SEO対策ルール】: 検索流入を増やすため、検索優先度が高いキーワードやトレンドのワードを、見出し（H2/H3）や本文中に自然に組み込むこと。
   - 【AEO対策ルール】: 記事の最後に「よくある質問（FAQ）」として、読者が検索エンジンに直接問いかけるようなQ&A（QとAのセット）を1〜2つ必ず含めること。回答はAI検索（AI Overviews等）に引用されやすいよう、ダラダラ書かずに「簡潔な結論」から書き始めること。
   - 【内部リンクの必須ルール】: LPのトップページ（ \`/\` ）や関連テーマへのリンク（例: \`[当院の詳細・ご相談はこちら](/)\` ）を、文脈に合わせて自然に2箇所以上必ず挿入すること。
