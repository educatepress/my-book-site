...
1. "jpBlog": 完全な日本語版のMDXブログ記事（2000文字程度）。
   ...
   - 【SEO・内部リンクの必須ルール】: LPのトップページ（ \`/\` ）や、「葉酸の大切さについて」など関連する可能性のあるテーマへのテキストリンク（例: \`[書籍の詳細はこちら](/)\` ）を、文脈に合わせて自然に2箇所以上必ず挿入すること。※クリニックは存在しないため「来院予約」や「当院」などの表現は一切使用しないでください。
   - 【AEO対策ルール】: 記事の最後に「よくある質問（FAQ）」として、このテーマに関して読者が抱きやすいQ&A（QとAのセット）を1〜2つ必ず含めること。
   - 記事の最後には、必ず以下の【書籍紹介CTA】のいずれか1つを自然な流れで挿入し、書籍リンク（ https://www.amazon.co.jp/dp/B0DV8Z3XZR?tag=ttcguide-blog-22 ）へ誘導すること。
...
2. "enBlog": 日本語版と同内容の、英語に翻訳・ローカライズされたMDXブログ記事。
   ...
   - 記事の最後には必ず以下のURLで英語版書籍へのCTAを含めること: https://www.amazon.com/dp/B0F771VNV5?tag=ttcguide-enblog-22
